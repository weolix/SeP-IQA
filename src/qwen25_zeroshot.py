...
class MultimodalQualityEvaluator(nn.Module):
    ...
    def forward(self, image_or_video=None, labels=None, **args):
        # image_or_video is the direct output from DataLoader (list of data_items)
        if image_or_video is None:
            raise ValueError("image_or_video must be provided.")
...
def evaluate_iqa_qwen(model, val_dataset, val_embed, q_bench_good_poor_tokens, bsz=2):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm.tqdm(valdataloader, desc="Qwen Validation", ncols=100, disable=None)):
            ...
            outputs = model(image_or_video=image_or_video_data_batch)
...
##############################################################
    ...
    get_top_logits("/home/ippl/xxr/mPLUG-Owl/mPLUG-Owl3/assets/testimg", model, batch_size=1, topk=20)