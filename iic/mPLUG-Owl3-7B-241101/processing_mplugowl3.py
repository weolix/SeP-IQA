...
class mPLUGOwl3Processor(ProcessorMixin):
    ...
    def build_text_qwen(self, messages, preface=False):
        ...
        if self.inference_mode and not preface:
    ...
    def encode_text_sft(self, texts):
        ...
        for current_ti, text_chunk in enumerate(texts):
            ...
            # For media tokens
                ...
                add_length = media_helper.add_media(
                    enc_chunk, text=text, tokenize_fn=self.wrapped_tokenize
                )
        ...
        media_offset = [
            torch.tensor([_[0] for _ in media_helper.media_position]).long()
        ]
        return {
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if videos is not None:
            medias.extend(
                [
                    {"type": "video", "content": video, "use_video_span": True}
                    for video in videos
                ]
            )
        ...
        if len(medias):
            ...
            assert image_token_ptr == len(medias), (
                image_token_ptr,
                len(medias),
            )  # 保证图和token数目一致
            ...
            assert num_image_tokens == num_image_shapes, (
                messages,
                [_.shape for _ in image_tensor_list],
            )
        ...
        text = self.build_text_qwen(messages, preface=preface)
    ...
    def check_media(self, images, messages):
        ...
        media_count = sum(
            [message["content"].count("<|image|>") for message in messages]
        )
